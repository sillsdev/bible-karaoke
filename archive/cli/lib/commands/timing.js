...
var Command = new utils.Resource({
  command: 'timing',
  params: '',
  descriptionShort: 'convert the text and timing files into an intermediate format.',
  descriptionLong: `
`,
});
    ...
    async.series(
      [
        ...
        (done) => {
          ...
          if (!Options.output) {
            Options.output = path.join(process.cwd(), 'bbkFormat.js');
          }
...
function convertIt(done) {
  ...
  var getDuration = (tObj, cb) => {
    ...
    var fileName = path.join(path.dirname(Options.input), `${tObj.index}.wav`);

    try {
      getAudioDurationInSeconds(fileName, Options.ffprobePath)
        .then((duration) => {
  ...
  var processLine = (lines, startTime, cb) => {
    ...
    if (lines.length == 0) {
      ...
      if (line.HeadingType && line.HeadingType._text == 'c') {
        ...
        var audioFile = allFiles.find((f) => {
          return f.indexOf(audioPartial) == 0;
        });
        if (audioFile) {
      ...
      getDuration(timingObj, (err) => {
        if (err) {
          ...
          formatWords(timingObj.content.split(' '), timingObj, timingObj.start, (err) => {
            if (err) {
              cb(err);
              return;
            }
            ...
            // next line
            processLine(lines, timingObj.start + timingObj.duration, cb);
          });
        }
      });
    }
...
function saveOutput(done) {
  fs.writeFile(Options.output, `module.exports = ${JSON.stringify(finalFormat, null, 4)}`, (err) => {
    if (err) {
