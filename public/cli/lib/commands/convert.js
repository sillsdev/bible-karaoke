    ...
    async.series(
      [
        ...
        (done) => {
          ...
          if (Options.pathFolders && !Array.isArray(Options.pathFolders)) Options.pathFolders = [Options.pathFolders];
  ...
  inquirer
    .prompt([
      ...
      },
      {
        ...
        default: false,
        when: (values) => {
          return !values.wantBG && typeof Options.bgFile == 'undefined' && typeof Options.noBGImage == 'undefined';
        },
      },
      {
        name: 'bgFile',
        type: 'input',
        message: 'Enter the path to the background image or video you want to use:',
        default: '',
        validate: (input) => {
          if (fs.existsSync(input)) {
            return true;
        ...
        when: (values) => {
          return values.wantBG && !values.bgFile && !Options.bgFile && typeof Options.noBGImage == 'undefined';
        },
      },
      {
        name: 'fontFamily',
        type: 'input',
        message: 'Enter the name of your system font to use in the video.',
      ...
      },
      {
        name: 'fontSize',
        type: 'input',
        message: 'Enter the font size (pt) to use in the video.',
        default: '20',
        when: (values) => {
      ...
      },
      {
        name: 'fontColor',
        type: 'input',
        message: 'Enter the font color to use in the video.',
        default: '#555',
        when: (values) => {
      ...
      },
      {
        ...
        type: 'input',
        message: 'Enter the background color to use in the speech bubble.',
      ...
      },
      {
        name: 'combineOutput',
        type: 'input',
        message: 'Do you want to combine chapters to one video ?',
        default: false,
        when: (values) => {
    ...
    tasks.push(
      () =>
        new Promise((next, fail) => {
          ...
          fs.writeFile(outputListFile, outputFiles.map((f) => `file '${f}'`).join('\n'), (err) => {
    ...
    tasks.push(
      () =>
        new Promise((next, fail) => {
          ...
          let ffmpegProcess = shell.spawn(ffmpegExe, [
            'f concat -safe 0 -i',
            outputListFile,
            '-c copy',
            Options.output,
