...
const { Project, Book, Chapter, getDirectories } = require('./util');
...
function makeBook(projectName, name, directory) {
  let book = new Book();
  book.name = name;
  let chapterNames = getDirectories(path.join(directory, projectName, name));

  let naturalSortChapterNames = chapterNames.sort(
...
function makeChapter(projectName, bookName, name, directory) {
  ...
  let chapterFiles = fs.readdirSync(path.join(directory, projectName, bookName, name));
  chapter.audioFiles = chapterFiles
    .filter((file) => file !== DEFAULT_XML_NAME)
    .map((fileName) => path.join(directory, projectName, bookName, name, fileName));

  chapter.textXmlFile = chapterFiles.find((file) => file === DEFAULT_XML_NAME);
  if (chapter.textXmlFile) chapter.textXmlFile = path.join(directory, projectName, bookName, name, chapter.textXmlFile);
